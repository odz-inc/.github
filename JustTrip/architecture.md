...
### 6. Environment Variables (Security-relevant)

```env
# Firebase Admin SDK credentials (JSON string)
FIREBASE_CONFIG={"type":"service_account","project_id":...}

# Set true for local dev — disables Firebase, enables mock auth
IS_LOCAL=false
...
```bash
cd engine && arq src.worker.WorkerSettings
```

### 6. Start MCP server (optional)
```bash
cd mcp_server && uvicorn src.server:app --port 5001
```
...
Defined in `api/src/routers/plans.py` and `api/src/routers/users.py`:

```
POST /api/v1/plans/generate          # Queue trip plan generation
GET  /api/v1/plans/status/{job_id}   # Poll job status
GET  /api/v1/plans/{plan_id}         # Get completed plan
POST /api/v1/plans/{plan_id}/feedback      # Record user edits (ML signal)
POST /api/v1/plans/{plan_id}/alternatives  # Generate Plan B
...
```env
# Database
DATABASE_URL=postgresql://user:pass@localhost:5432/travelapp
...
# Firebase (not needed when IS_LOCAL=true)
FIREBASE_ADMIN_KEY_PATH=keys/firebase-adminsdk.json

# Local mode — skips Firebase auth
IS_LOCAL=true
```
